...
def _to_identifiers(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    *,
...
) -> list[tuple[ParameterId, ParameterId]]:
    if not input_parameter_is_time_series:
        input_identifier = StaticParameterId(name=input_parameter_ref.name)
        if not output_parameter_is_time_series:
            output_identifiers: list[ParameterId] = [StaticParameterId(name=output_parameter_ref.name)]
        elif output_parameter_ref.next:
            output_identifiers = [
                TimeSeriesParameterId(name=output_parameter_ref.name, time_step=time_step)
                for time_step in range(1, n_time_steps)
            ]
        elif output_parameter_ref.initial:
            output_identifiers = [TimeSeriesParameterId(name=output_parameter_ref.name, time_step=0)]
            ...
            match (relationship_includes_past, relationship_includes_future):
                ...
                case _, _:
                    ...
                    raise ValueError(msg)
            output_identifiers = [
                TimeSeriesParameterId(name=output_parameter_ref.name, time_step=time_step) for time_step in time_steps
            ]
        else:
    ...
    if not output_parameter_is_time_series:
        output_identifier = StaticParameterId(name=output_parameter_ref.name)
        if input_parameter_ref.all_time_steps:
            input_identifiers = [
                TimeSeriesParameterId(name=input_parameter_ref.name, time_step=time_step)
                for time_step in range(n_time_steps)
            ]
        else:
    ...
    if input_parameter_ref.normal and output_parameter_ref.normal:
        ...
        input_identifiers = [
            TimeSeriesParameterId(name=input_parameter_ref.name, time_step=time_step) for time_step in time_steps
        ]
        output_identifiers = [
            TimeSeriesParameterId(name=output_parameter_ref.name, time_step=time_step) for time_step in time_steps
        ]
...
def _exec_relationship(  # noqa: C901, PLR0912
    ...
    *,
    ...
    if isinstance(output_to_include, TimeSeriesParameterId):
    ...
    if relationship_phase == RelationshipPhase.IN_PROPAGATION:
        assert isinstance(output_to_include, TimeSeriesParameterId)
        ...
        input_parameter_ref_to_identifier: dict[ParameterRef, ParameterId] = {}
        for parameter_ref in relationship.iter_input_parameter_refs():
            if parameters[parameter_ref.name].is_time_series:
                input_parameter_ref_to_identifier[parameter_ref] = TimeSeriesParameterId(
                    name=parameter_ref.name,
                    time_step=relationship_time_step - 1 if parameter_ref.previous else relationship_time_step,
                )
            else:
                input_parameter_ref_to_identifier[parameter_ref] = StaticParameterId(name=parameter_ref.name)
        input_values = {
        ...
        output_parameter_ref_to_identifier: dict[ParameterRef, ParameterId] = {}
        for parameter_ref in relationship.iter_output_parameter_refs():
            if parameters[parameter_ref.name].is_time_series:
                output_parameter_ref_to_identifier[parameter_ref] = TimeSeriesParameterId(
                    name=parameter_ref.name,
                    time_step=relationship_time_step + 1 if parameter_ref.next else relationship_time_step,
                )
            else:
                output_parameter_ref_to_identifier[parameter_ref] = StaticParameterId(name=parameter_ref.name)
        return {
    ...
    for parameter_ref in relationship.iter_input_parameter_refs():
        if parameters[parameter_ref.name].is_time_series:
            ...
            input_values[parameter_ref] = [
                inputs[TimeSeriesParameterId(name=parameter_ref.name, time_step=time_step)]
                for time_step in range(n_time_steps)
            ]
        else:
            input_values[parameter_ref] = inputs[StaticParameterId(name=parameter_ref.name)]
    relationship_outputs = _exec_relationship_inner(relationship, input_values)
    output_by_identifier: dict[ParameterId, Any] = {}
    for parameter_ref, output_value in relationship_outputs.items():
        if parameters[parameter_ref.name].is_time_series:
            if parameter_ref.all_time_steps:
                output_by_identifier.update(
                    {
                        TimeSeriesParameterId(name=parameter_ref.name, time_step=time_step): output_value[time_step]
                        for time_step in range(n_time_steps)
                    },
                )
            else:
                assert parameter_ref.initial
                output_by_identifier[TimeSeriesParameterId(name=parameter_ref.name, time_step=0)] = output_value
        else:
            output_by_identifier[StaticParameterId(name=parameter_ref.name)] = output_value
...
def exec_core_model(  # noqa: C901, PLR0912
    ...
    *,
    ...
    parameter_identieifers: set[ParameterId] = {
        StaticParameterId(name=param_name)
        for param_name, parameter in core_model.parameters.items()
        if not parameter.is_time_series
    } | {
        TimeSeriesParameterId(name=param_name, time_step=time_step)
        for param_name, parameter in core_model.parameters.items()
        if parameter.is_time_series
        for time_step in range(n_time_steps)
    }
    identifier_to_relationship: dict[ParameterId, str] = {}

    relationships_without_inputs: set[RelationshipABC] = set()
    relationships_without_outputs: set[RelationshipABC] = set()
    dependencies_dd: defaultdict[ParameterId, set[ParameterId]] = defaultdict(set)  # input -> [output]
        ...
        if identifier not in identifier_to_relationship:
            ...
            for input_parameter_ref in inputs:
                if input_parameter_ref.name != identifier.name:
                    continue
                if isinstance(identifier, StaticParameterId) and input_parameter_ref.normal:
                    results[identifier] = inputs[input_parameter_ref]
                    break
                if (
                    isinstance(identifier, TimeSeriesParameterId)
                    and identifier.time_step == 0
                    and input_parameter_ref.initial
                ):
                    results[identifier] = inputs[input_parameter_ref]
                    break
                if isinstance(identifier, TimeSeriesParameterId) and input_parameter_ref.all_time_steps:
